...
export default function UserMenu({ user }: { user: User }) {
  ...
  // 1. Calcul du Nom à afficher
  // Priorité : Métadonnée (Nom complet) > Email (Partie avant le @) > 'Utilisateur'
  const displayName = user.user_metadata?.full_name || user.email?.split('@')[0] || 'Utilisateur';

  // 2. Calcul de l'Initiale
  const initial = displayName.charAt(0).toUpperCase();

  return (
    <div className="relative" ref={menuRef}>
      {/* AVATAR ROND CLIQUABLE */}
      <button 
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 py-2 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="px-4 py-3 border-b border-gray-100 bg-gray-50/50">
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wider mb-1">Connecté en tant que</p>
